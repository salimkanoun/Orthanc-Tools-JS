...
export default ({ studies, canExpand = false, renderSubComponent = undefined, onRowClick = undefined, additionalColumns = [] }) => {
    ...
    return (
        <CommonTableV8 onRowClick={onRowClick} renderSubComponent={renderSubComponent} canExpand={canExpand} id={studyQueryColumns.STUDY_INSTANCE_UID.accessorKey} columns={[...columns, ...additionalColumns]} data={studies} canSort canFilter paginated />
    )
}