...
class RetrieveTask {
  ...
  static async createTask(creator, projectName, answers) {
    let task = await RetrieveTask.getUserTask(creator).then((ids) =>
      ids ? RetrieveTask.getTask(ids[0]) : null
    );
    // Checking for existing task of the user
    if (!!task && !["completed", "failed"].includes(task.state)) {
      throw new OTJSForbiddenException(
        "Cant create two retrieval simulteanously"
      );
    }
    ...
    let curratedItems = items.reduce((aggregation, item) => {
      if (item.Level === OrthancQueryAnswer.LEVEL_STUDY) {
        for (const existingItem of aggregation) {
          if (item.StudyInstanceUID === existingItem.StudyInstanceUID)
            return aggregation;
        }
        ...
        for (const existingItem of aggregation) {
          if (
            ...
            item.SeriesInstanceUID === existingItem.SeriesInstanceUID
          )
            return aggregation;
        }
        aggregation.push(item);
  ...
  static async validateTask(id) {
    ...
    if (task.progress.validation != 100)
      throw OTJSBadRequestException("Items validation still in progress");
  ...
  static async getTask(id) {
    ...
    let validationJobs = (await RetrieveTask._getValidationJobs(id)).sort(
      sorter
    );
    ...
    if (progress.validation < 100) {
      ...
      progress.validation === 100 &&
      progress.retrieve === 0 &&
      retrieveJobs.length === 0
    ) {
      state = "waiting admin validation";
    } else if (
      progress.validation === 100 &&
      progress.retrieve < 100 &&
      retrieveJobs.length !== 0
    ) {
      state = "retrieving";
    } else if (
    ...
    for (let i = 0; i < validationJobs.length; i++) {
      ...
      let Validated =
        (await validateJob.getState()) === "completed"
          ? await validateJob.finished()
          : null;
      ...
      items.push({
        ...
        Status: state,
        RetrievedOrthancId:
          state === "completed" ? await retrieveJob.finished() : null,
      });
    }
  ...
  static async getUserTask(user) {
    let validateJobs = await validationQueue
      .getJobs()
  ...
  static async deleteItem(taskId, itemId) {
    ...
    if (retrieveJobs.length !== 0)
      throw new OTJSForbiddenException(
        "Can't delete a robot already in progress"
      );
    let validateJobs = await RetrieveTask._getValidationJobs(taskId);
    let answerId = itemId.split(":")[1];
    let answerNumber = itemId.split(":")[0];
    let job = validateJobs.filter(
      (job) =>
        job.data.item.AnswerNumber == answerNumber &&
        job.data.item.AnswerId == answerId
    )[0];
  ...
  static async retryItem(taskId, itemId) {
    ...
    if (state !== "failed")
      throw new OTJSBadRequestException("Can't retry a job that isn't failed");
    await job.retry();
  }
  ...
  static async delete(taskId) {
    ...
    let stateComplete = (
      await Promise.all(retrieveJobs.map((job) => job.getState()))
    ).reduce((acc, x) => acc && (x === "completed" || x === "failed"), true);

    if (retrieveJobs.length !== 0 && !stateComplete)
      throw new OTJSForbiddenException(
        "Can't delete a robot already in progress"
      );
    let validateJobs = await RetrieveTask._getValidationJobs(taskId);
  ...
  static async decay(user) {
    let validateJobs = await validationQueue
      .getJobs()
      .then((jobs) => jobs.filter((job) => job.data.creator === user));
    let retrieveJobs = await retrieveQueue
      .getJobs()
      .then((jobs) => jobs.filter((job) => job.data.creator === user));
  ...
  static async _validateItem(job, done) {
    try {
      ...
      //Querry the dicom
      const answerDetails = await orthanc.makeDicomQuery(
        job.data.item.OriginAET
      );
  ...
  static async _retrieveItem(job, done) {
    ...
    try {
      ...
      const retrieveAnswer = await orthanc.makeRetrieve(
        answer.AnswerId,
        answer.AnswerNumber,
        await orthanc.getOrthancAetName(),
        false
      );
      ...
      await orthanc
        ...
        .then(async (response) => {
          const query = response.Content["Query"][0];
          //If series UID present (prioritary as indicate series retrieve), use studyinstanceUID otherwise
          let orthancResults = null;
          if (query["0020,000e"]) {
            orthancResults = await orthanc.findInOrthancBySeriesInstanceUID(
              query["0020,000e"]
            );
          } else {
            orthancResults = await orthanc.findInOrthancByUid(
              query["0020,000d"]
            );
          }
...
async function setupRetrieveSchedule() {
  ...
  if (
    time.isTimeInbetween(
      now.getHours(),
      now.getMinutes(),
      optionsParameters.hour_start,
      optionsParameters.min_start,
      optionsParameters.min_stop,
      optionsParameters.hour_stop
    )
  ) {
  ...
  this.resumer = schedule.scheduleJob(
    optionsParameters.min_start + " " + optionsParameters.hour_start + " * * *",
    () => {
      retrieveQueue.resume();
    }
  );
  this.pauser = schedule.scheduleJob(
    optionsParameters.min_stop + " " + optionsParameters.hour_stop + " * * *",
    () => {
      retrieveQueue.pause();
    }
  );
...
let validationQueue = new Queue(
  "validation",
  RetrieveTask._validateItem,
  Number(process.env.RETRIEVE_ATTEMPTS) || 3,
  Number(process.env.RETRIEVE_BACKOFF) || 2000,
  Number(process.env.RETRIEVE_WORKERS) || 3
);
let retrieveQueue = new Queue(
  "retrieve",
  RetrieveTask._retrieveItem,
  Number(process.env.RETRIEVE_ATTEMPTS) || 3,
  Number(process.env.RETRIEVE_BACKOFF) || 2000,
  Number(process.env.RETRIEVE_WORKERS) || 3
);
